# Configurações Avançadas de Animação — Framer Motion

## Objetivo
Este arquivo centraliza a lógica de animações fluidas da landing page do NanoBox. As configurações abaixo foram desenhadas para imitar as transições nativas de interfaces de alta fidelidade como Apple e Linear, garantindo alta performance no motor de renderização.

---

## Definição de Variantes Reutilizáveis (Código de Referência)

```javascript
import { Variants } from "framer-motion";

// Animação de revelação com desfoque e subida suave (Ideal para Títulos e Badges)
export const fadeBlurReveal: Variants = {
  hidden: { 
    opacity: 0, 
    y: 25, 
    filter: "blur(8px)" 
